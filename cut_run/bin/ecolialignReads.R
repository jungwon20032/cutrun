...
align.experimental <- function(row) {
	...
	command <- paste(
		"bowtie2",
		"-x", file.path(opt$outdir, "ecoli_genome", "index", "ecoli_genome"),
		"-S", file.path(opt$outdir, "ecoli_aligned", paste0(row["sample_ID"], "_", row["condition"], "_", row["replicate"], ".sam")),
...
align.control <- function(row) {
	...
	command <- paste(
		"bowtie2",
		"-x", file.path(opt$outdir, "ecoli_genome", "index", "ecoli_genome"),
		"-S", file.path(opt$outdir, "ecoli_aligned", paste0(row["control_ID"], ".sam")),
		"-q --phred33 --no-unal --threads", opt$threads
	)
	# adding paired versus unpaired options
